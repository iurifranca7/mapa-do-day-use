...
const PaymentSection = ({
  ...
  // 🔥 LÓGICA DE PARCELAS DO MERCADO PAGO
  useEffect(() => {
    ...
    if (mpInstance && finalTotal > 0 && bin && bin.length >= 6) {
      ...
      .then((response) => {
        if (response.length > 0) {
          ...
          // 🛠️ CORREÇÃO CRÍTICA: Define o Issuer (Banco) e a Bandeira aqui
          // Isso evita o erro "installments_excludes_country"
          ...
          // Removido o filtro de 5 parcelas conforme solicitado
          // O Mercado Pago vai retornar todas as opções disponíveis (ex: até 12x)
          setInstallmentOptions(payerCosts);
          
          // Se a parcela selecionada anteriormente não existe mais na nova lista, reseta para 1x
          if (!installments || !payerCosts.find(opt => opt.installments === Number(installments))) {
              setInstallments(1);
          }
        } else {
                ...
                {mpInstance ? (
                    <div className="space-y-5">
                        <CreditCardForm 
                            ...
                            // 🔥 CORREÇÃO: Adicionado o evento para capturar o BIN e disparar o cálculo
