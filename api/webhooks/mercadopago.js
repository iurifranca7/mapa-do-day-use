...
// ==================================================================
// 1. INICIALIZAÇÃO DO FIREBASE (Reutilizada e Segura)
// ==================================================================
const initFirebase = () => {
    if (!admin || !admin.apps) {
...
const notifyCustomer = async (reservationData, reservationId) => {
    ...
    try {
        ...
        const mapLink = `http://maps.google.com/maps?q=${encodeURIComponent(reservationData.item.name + " " + reservationData.item.city)}`;
        ...
        // Tenta usar variável de ambiente ou fallback para localhost em dev
...
// ==================================================================
// 3. HANDLER PRINCIPAL DO WEBHOOK
// ==================================================================
export default async function handler(req, res) {
    ...
    // Filtra apenas eventos de pagamento atualizados
    ...
    try {
        ...
        // 1. Encontra a Reserva pelo PaymentID
        ...
        if (snapshot.empty) {
            console.warn(`Webhook: Nenhuma reserva encontrada para o pagamento ${paymentId}`);
            return res.status(200).send('Reservation not found');
        }
        ...
        // 2. Busca Token do Parceiro para validar status no MP
        const ownerSnap = await db.collection('users').doc(reservationData.ownerId).get();
        const partnerAccessToken = ownerSnap.data()?.mp_access_token;

        if (!partnerAccessToken) {
            console.error('Webhook: Token do parceiro não encontrado.');
            return res.status(200).send('Partner token missing');
        }

        // 3. Consulta Oficial no Mercado Pago
        ...
        console.log(`Webhook MP: Pagamento ${paymentId} está ${statusMP} (${statusDetail})`);

        // 4. Lógica de Atualização
        let newStatus = reservationData.status;
        let updateData = { 
            updatedAt: new Date(), 
            mpStatus: statusMP, 
            mpDetail: statusDetail 
        };

        switch (statusMP) {
            case 'approved':
                // Se o pagamento foi aprovado agora (ex: Pix pago depois ou Cartão aprovado)
                // e a reserva ainda não estava confirmada
                if (reservationData.status !== 'confirmed') {
                    newStatus = 'confirmed';
                    updateData.approvedAt = new Date();
                    
                    // IMPORTANTE: Dispara o E-mail agora!
                    // Precisamos garantir que 'item' existe em reservationData. 
                    // Se você salvou o 'item' completo na criação, isso vai funcionar.
                    if (reservationData.item) {
                        await notifyCustomer(reservationData, reservationId);
                    }
                }
                break;

            case 'charged_back': 
                // CRÍTICO: Cliente contestou a compra
                newStatus = 'chargeback'; 
                // Flag para alerta no painel do parceiro
                updateData.alert = "⚠️ Contestação (Chargeback) iniciada pelo titular do cartão.";
                break;

            case 'cancelled': 
            case 'rejected':  
            case 'refunded':  
                // Só cancela se ainda não estiver cancelado
                if (reservationData.status !== 'cancelled') {
                    newStatus = 'cancelled';
                    updateData.cancelledAt = new Date();
                    updateData.cancelReason = statusMP === 'refunded' ? 'Estorno Mercado Pago' : 'Cancelamento Mercado Pago';
                }
                break;
        }

        // 5. Salva no Banco se mudou o status ou tem dados novos
        if (newStatus !== reservationData.status || updateData.alert) {
            updateData.status = newStatus;
            await db.collection('reservations').doc(reservationId).update(updateData);
            console.log(`✅ Reserva ${reservationId} atualizada via Webhook: ${newStatus}`);
        }
